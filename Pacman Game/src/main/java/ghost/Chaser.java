...
public class Chaser extends Ghost {
    ...
    private final int[] HOME = {1,4}; // Default place it need to go to when in scatter mode.

    public Chaser(PApplet pApplet, char[][] map, Waka waka, char name) {
        ...
        pathScatter = (new AStar(getMap(), x / PIXEL_SIZE, y / PIXEL_SIZE, false)).findPathTo(HOME[0], HOME[1]);
    }
    ...
    public String getImageFileName() {
        return "src/main/resources/chaser.png";
    }

    /**
     * The ghost target location is Waka and it will always chase until hit Waka.
     *
     * @return direction it needs to chase waka
     */
    public Direction chase() {
        int[] wakaPos = waka.getWakaPixelPos();
    ...
    /**
     * The ghost target location is the top left corner.
     *
     * @return the direction it need to go to top left corner.
     */
    public Direction scatter() {
        ...
        // The pathChase will return a list of way the ghost need to move, but we need only the first element of the list
        // to define the next direction that the ghost need to move
        // for example if Waka is at position (0,2) and the ghost is at (0,0) the pathchase will return a list {[0,0],[0,1],[0,2]}
        if (!reachCorner) {
            ...
            Direction[] values = {Direction.down, Direction.right}; // To minimize the chance that the random direction is always go up or right continuously.
    ...
    /**
     * Declare whether chaser is at the top corner ot not.
     *
